...
for subject_dir in os.listdir(args.subjects_root_path):
    ...
    try:
        ...
        sys.stdout.write(
            'got {0} stays, {1} diagnoses, {2} events...'.format(stays.shape[0], diagnoses.shape[0], events.shape[0]))
        sys.stdout.flush()
    ...
    for i in range(stays.shape[0]):
        ...
        episodic_data.ix[episodic_data.index == stay_id].to_csv(
            os.path.join(args.subjects_root_path, subject_dir, 'episode{}_readmission.csv'.format(i + 1)),
            index_label='Icustay')
        columns = list(episode.columns)
        columns_sorted = sorted(columns, key=(lambda x: "" if x == "Hours" else x))
        episode = episode[columns_sorted]
        episode.to_csv(
            os.path.join(args.subjects_root_path, subject_dir, 'episode{}_timeseries_readmission.csv'.format(i + 1)),
            index_label='Hours')
