...
if args.verbose:
    print('transfers_REMOVE PATIENTS AGE < 18:', transfers.ICUSTAY_ID.unique().shape[0],
          transfers.HADM_ID.unique().shape[0],
          transfers.SUBJECT_ID.unique().shape[0])
...
phenotypes = add_hcup_ccs_2015_groups(diagnoses,
                                      yaml.load(open(args.phenotype_definitions, 'r'), Loader=yaml.FullLoader))
