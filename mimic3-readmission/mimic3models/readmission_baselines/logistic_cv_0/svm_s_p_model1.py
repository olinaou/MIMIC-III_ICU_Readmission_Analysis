...
def svm_f_importances_top(coef, names, top=-1):
    imp = coef
    imp, names = zip(*sorted(list(zip(imp, names))))
    print("====> important features: " + str(zip(imp, names)))
    ...
    # plt.yticks(range(top),  ['LOS', 'LOS_HOSPITAL', 'Glascow coma scale motor response'])
    plt.show()
...
additional_features = get_additional_features(val_names)
...
features_val_extended = [np.hstack([X, d]) for (X, d) in zip(features_val, additional_features.values.tolist())]
...
additional_features = get_additional_features(test_names)
...
features_test_extended = [np.hstack([X, d]) for (X, d) in zip(features_test, additional_features.values.tolist())]
...
logreg = SVC(probability=True, kernel='linear')
...
svm_features_names = [str(i) for i in range(0, 391)]
# svm_features_names = ['LOS', 'LOS_HOSPITAL', 'Glascow coma scale motor response']
svm_f_importances_top(abs(logreg.coef_[0]), svm_features_names, 10)
