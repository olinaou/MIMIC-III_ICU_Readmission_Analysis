...
def load_data_model1(reader, discretizer, normalizer, diseases_embedding, demographic, additional_features, small_part=False, return_names=False):
    N = reader.get_number_of_examples()
    if small_part:
        N = 1000
    ret = common_utils.read_chunk(reader, N)
    ...
    data = [np.hstack([X, [d] * len(X)]) for (X, d) in zip(data, diseases_embedding)]
    data = [np.hstack([X, [d] * len(X)]) for (X, d) in zip(data, demographic)]
    data = [np.hstack([X, [d] * len(X)]) for (X, d) in zip(data, additional_features)]
...
def load_train_data(reader, discretizer, normalizer, diseases_embedding, demographic, small_part=False,
                    return_names=False):
    N = reader.get_number_of_examples()
...
def load_train_data_model1(reader, discretizer, normalizer, diseases_embedding, demographic, additional_features, small_part=False,
                    return_names=False):
    N = reader.get_number_of_examples()
